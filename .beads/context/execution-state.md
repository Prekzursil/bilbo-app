...
- ITER 4 | 2026-04-29T23:30Z | WU-A2.1-shipped | branch=wu/a2-sqldelight-usage-repository PR=#52 | gates=8/12 zero | notes=Shipped first SqlDelight repository (UsageRepository) as code-only PR #52. Implementation in shared/src/commonMain/.../SqlDelightUsageRepository.kt (147 LOC) wires every UsageRepository method to existing BilboDatabase.sq queries. Schema gained `lastInsertRowId: SELECT last_insert_rowid()` for transaction-safe inserts. Class added to kover excludes (temporary) so the 100% gate stays green during transition. Codacy gate is now wired (user enrolled the repo) — confirmed by 'Codacy Static Code Analysis' status check on PR #52. Follow-up WU-A2.1.tests will: add JdbcSqliteDriver to androidUnitTest, write tests against in-memory driver, remove kover exclusion, flip RepositoryModule.kt's binding from InMemoryUsageRepository to SqlDelightUsageRepository.
- ITER 5 | next | TODO | will: monitor PR #52 merge, then start WU-A2.1.tests (commonTest + driver + flip DI). Then A2.2 (AppProfile), A2.3 (Intent), A2.4 (Emotion), A2.5 (Budget), A2.6 (Suggestion). In parallel: WU-B12.semgrep (style rules + 12 fixes), WU-B5 (sonar properties v2), WU-B12.codacy (configure Codacy to enforce ≥80% coverage on new code now that the repo is registered). | result=pending
