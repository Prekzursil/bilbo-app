...
class NightlyAnalysisWorker @AssistedInject constructor(
    ...
    companion object {
        const val WORK_NAME = "spark_nightly_analysis"
    ...
    private fun buildCorrelationSummary(
        ...
        val stressedCount = checkIns.count {
            it.preSessionEmotion == dev.spark.domain.Emotion.STRESSED ||
                it.preSessionEmotion == dev.spark.domain.Emotion.ANXIOUS
        }.toFloat()
        val totalCheckIns = checkIns.size.toFloat()

        val emptyCalorieSessions = sessions.filter {
            it.category == dev.spark.domain.AppCategory.EMPTY_CALORIES
        }
