...
//
// Covers: BypassManager, AppInfo, AppMonitor, SparkError, DefaultErrorHandler,
...
import dev.spark.data.ResourceReader
import dev.spark.data.SeedDataLoader
import dev.spark.data.SeedPreferenceStore
...
import dev.spark.preferences.DefaultEnforcementMode
import dev.spark.preferences.NotificationPreferences
import dev.spark.preferences.SharingLevelPref
...
import dev.spark.util.SparkError
import dev.spark.util.DefaultErrorHandler
import dev.spark.util.ErrorListener
import dev.spark.util.NetworkException
import dev.spark.util.OfflineException
import dev.spark.util.safeCall
import dev.spark.util.toUserMessage
import dev.spark.util.withRetry
...
// =============================================================================
// MARK: - BypassManager Tests (production class at dev.spark.tracking.BypassManager)
// =============================================================================

class BypassManagerProductionTest {

    private lateinit var manager: dev.spark.tracking.BypassManager

    @BeforeTest
    fun setUp() {
        manager = dev.spark.tracking.BypassManager()
    }
    ...
    fun defaultBypassPackagesIsNotEmpty() {
        assertTrue(dev.spark.tracking.BypassManager.DEFAULT_BYPASS_PACKAGES.isNotEmpty())
    }

    @Test
    fun defaultBypassListContainsAndroidDialer() {
        assertTrue("com.android.dialer" in dev.spark.tracking.BypassManager.DEFAULT_BYPASS_PACKAGES)
    }

    @Test
    fun defaultBypassListContainsGoogleDialer() {
        assertTrue("com.google.android.dialer" in dev.spark.tracking.BypassManager.DEFAULT_BYPASS_PACKAGES)
    }

    @Test
    fun defaultBypassListContainsSamsungDialer() {
        assertTrue("com.samsung.android.dialer" in dev.spark.tracking.BypassManager.DEFAULT_BYPASS_PACKAGES)
    }

    @Test
    fun defaultBypassListContainsMmsApp() {
        assertTrue("com.android.mms" in dev.spark.tracking.BypassManager.DEFAULT_BYPASS_PACKAGES)
    }

    @Test
    fun defaultBypassListContainsGoogleMaps() {
        assertTrue("com.google.android.apps.maps" in dev.spark.tracking.BypassManager.DEFAULT_BYPASS_PACKAGES)
    }

    @Test
    fun defaultBypassListContainsWaze() {
        assertTrue("com.waze" in dev.spark.tracking.BypassManager.DEFAULT_BYPASS_PACKAGES)
    }

    @Test
    fun defaultBypassListContainsAndroidCamera() {
        assertTrue("com.android.camera" in dev.spark.tracking.BypassManager.DEFAULT_BYPASS_PACKAGES)
    }

    @Test
    fun defaultBypassListContainsAndroidSettings() {
        assertTrue("com.android.settings" in dev.spark.tracking.BypassManager.DEFAULT_BYPASS_PACKAGES)
    }

    @Test
    fun defaultBypassListContainsBilboApp() {
        assertTrue("dev.spark.app" in dev.spark.tracking.BypassManager.DEFAULT_BYPASS_PACKAGES)
    }

    @Test
    fun defaultBypassListContainsBilboDebug() {
        assertTrue("dev.spark.app.debug" in dev.spark.tracking.BypassManager.DEFAULT_BYPASS_PACKAGES)
    }

    @Test
    fun defaultBypassListContainsBilboGithub() {
        assertTrue("dev.spark.app.github" in dev.spark.tracking.BypassManager.DEFAULT_BYPASS_PACKAGES)
    }

    @Test
    fun defaultBypassListContainsSystemUi() {
        assertTrue("com.android.systemui" in dev.spark.tracking.BypassManager.DEFAULT_BYPASS_PACKAGES)
    }

    @Test
    fun defaultBypassListContainsEmergency() {
        assertTrue("com.android.emergency" in dev.spark.tracking.BypassManager.DEFAULT_BYPASS_PACKAGES)
    }

    @Test
    fun defaultBypassListContainsLauncher3() {
        assertTrue("com.android.launcher3" in dev.spark.tracking.BypassManager.DEFAULT_BYPASS_PACKAGES)
    }

    @Test
    fun defaultBypassListContainsDeskclock() {
        assertTrue("com.android.deskclock" in dev.spark.tracking.BypassManager.DEFAULT_BYPASS_PACKAGES)
    }

    @Test
    fun defaultBypassListContainsCalculator() {
        assertTrue("com.android.calculator2" in dev.spark.tracking.BypassManager.DEFAULT_BYPASS_PACKAGES)
    }
    ...
    fun setUserBypassWithEmptySetRestoresDefaults() {
        ...
        assertTrue(manager.shouldBypass("com.android.dialer"))
        assertFalse(manager.shouldBypass("com.custom.app"))
        assertEquals(dev.spark.tracking.BypassManager.DEFAULT_BYPASS_PACKAGES.size, manager.getBypassList().size)
    }
    ...
    fun getBypassListReturnsAllDefaults() {
        ...
        for (pkg in dev.spark.tracking.BypassManager.DEFAULT_BYPASS_PACKAGES) {
    ...
    fun freshInstanceHasOnlyDefaults() {
        val fresh = dev.spark.tracking.BypassManager()
        assertEquals(dev.spark.tracking.BypassManager.DEFAULT_BYPASS_PACKAGES, fresh.getBypassList())
    }

    @Test
    fun separateInstancesAreIndependent() {
        val mgr1 = dev.spark.tracking.BypassManager()
        val mgr2 = dev.spark.tracking.BypassManager()
...
class SparkErrorTest {
    ...
    fun clientErrorStoresStatusCodeAndMessage() {
        val error = SparkError.ClientError(400, "Bad Request")
    ...
    fun unknownWithCause() {
        ...
        val error = SparkError.Unknown(cause = cause)
        assertEquals(cause, error.cause)
    }

    @Test
    fun unknownCustomMessage() {
        val error = SparkError.Unknown(message = "Oops")
        assertEquals("Oops", error.message)
    }

    @Test
    fun bilboErrorIsException() {
        val error: Exception = SparkError.Offline()
        assertTrue(error is Exception)
    }

    @Test
    fun bilboErrorSubtypesAreDistinct() {
        val offline = SparkError.Offline()
        val server = SparkError.ServerError(500)
        val client = SparkError.ClientError(400, "Bad")
        val unauthorized = SparkError.Unauthorized()
        val notFound = SparkError.NotFound()
        val dataError = SparkError.DataError()
        val unknown = SparkError.Unknown()
    ...
    @Test
    fun offlineEquality() {
        assertEquals(SparkError.Offline(), SparkError.Offline())
    }

    @Test
    fun serverErrorEqualitySameCode() {
        assertEquals(SparkError.ServerError(500), SparkError.ServerError(500))
    }

    @Test
    fun serverErrorInequalityDifferentCode() {
        assertNotEquals(SparkError.ServerError(500), SparkError.ServerError(502))
    }
}
...
class DefaultErrorHandlerTest {
    ...
    @Test
    fun mapSparkErrorReturnsItself() {
        val original = SparkError.Offline()
    ...
    fun mapBilboServerErrorReturnsItself() {
        val original = SparkError.ServerError(500)
        val mapped = handler.map(original)
        assertSame(original, mapped)
    }
    ...
    fun mapBilboClientErrorReturnsItself() {
        val original = SparkError.ClientError(400, "Bad Request")
        val mapped = handler.map(original)
        assertSame(original, mapped)
    }
    ...
    fun mapBilboUnauthorizedReturnsItself() {
        val original = SparkError.Unauthorized()
        val mapped = handler.map(original)
        assertSame(original, mapped)
    }
    ...
    fun mapBilboNotFoundReturnsItself() {
        val original = SparkError.NotFound()
        val mapped = handler.map(original)
        assertSame(original, mapped)
    }
    ...
    fun mapBilboDataErrorReturnsItself() {
        val original = SparkError.DataError()
        val mapped = handler.map(original)
        assertSame(original, mapped)
    }
    ...
    fun mapBilboUnknownReturnsItself() {
        val original = SparkError.Unknown()
        val mapped = handler.map(original)
        assertSame(original, mapped)
    }
    ...
    fun mapNetworkException404ReturnsNotFound() {
        ...
        assertTrue(mapped is SparkError.NotFound)
    }

    @Test
    fun mapNetworkException400ReturnsClientError() {
        val mapped = handler.map(NetworkException(400, "Bad Request"))
        assertTrue(mapped is SparkError.ClientError)
        assertEquals(400, (mapped as SparkError.ClientError).statusCode)
    }
    ...
    fun mapNetworkException422ReturnsClientError() {
        ...
        assertTrue(mapped is SparkError.ClientError)
        assertEquals(422, (mapped as SparkError.ClientError).statusCode)
    ...
    fun mapNetworkException429ReturnsClientError() {
        ...
        assertTrue(mapped is SparkError.ClientError)
        assertEquals(429, (mapped as SparkError.ClientError).statusCode)
    }
    ...
    fun mapNetworkException499ReturnsClientError() {
        ...
        assertTrue(mapped is SparkError.ClientError)
        assertEquals(499, (mapped as SparkError.ClientError).statusCode)
    }

    @Test
    fun mapNetworkException500ReturnsServerError() {
        val mapped = handler.map(NetworkException(500, "Internal Server Error"))
        assertTrue(mapped is SparkError.ServerError)
        assertEquals(500, (mapped as SparkError.ServerError).statusCode)
    }
    ...
    fun mapNetworkException502ReturnsServerError() {
        ...
        assertTrue(mapped is SparkError.ServerError)
        assertEquals(502, (mapped as SparkError.ServerError).statusCode)
    }
    ...
    fun mapNetworkException503ReturnsServerError() {
        ...
        assertTrue(mapped is SparkError.ServerError)
        assertEquals(503, (mapped as SparkError.ServerError).statusCode)
    }
    ...
    fun mapNetworkExceptionOtherStatusCodeReturnsUnknown() {
        ...
        assertTrue(mapped is SparkError.Unknown)
    }
    ...
    fun mapSerializationExceptionReturnsDataError() {
        ...
        assertTrue(mapped is SparkError.DataError)
    ...
    fun mapNetworkExceptionWithNullMessage4xxUsesDefault() {
        ...
        val mapped = handler.map(ex)
        assertTrue(mapped is SparkError.ClientError)
        assertEquals("I'm a teapot", (mapped as SparkError.ClientError).message)
    }
    ...
    fun handleMapsAndReturnsError() {
        ...
        assertTrue(result is SparkError.Offline)
    }

    @Test
    fun handleNotifiesListeners() {
        var received: SparkError? = null
        handler.addListener(ErrorListener { received = it })
        handler.handle(OfflineException())
        assertTrue(received is SparkError.Offline)
    }
    ...
    fun handleWithNoListenersDoesNotThrow() {
        ...
        assertTrue(result is SparkError.Unknown)
    }
...
class WithRetryTest {
    ...
    fun withRetryThrowsOnClientError() = runTest {
        assertFailsWith<SparkError.ClientError> {
            withRetry(
                maxAttempts = 3,
                initialDelay = 1L,
                factor = 1.0
    ...
    fun withRetryThrowsOnUnauthorized() = runTest {
        assertFailsWith<SparkError.Unauthorized> {
            withRetry(
                maxAttempts = 3,
                initialDelay = 1L,
                factor = 1.0
    ...
    fun withRetryThrowsOnNotFound() = runTest {
        assertFailsWith<SparkError.NotFound> {
            withRetry(
                maxAttempts = 3,
                initialDelay = 1L,
                factor = 1.0
    ...
    fun withRetryThrowsOnDataError() = runTest {
        assertFailsWith<SparkError.DataError> {
            withRetry(
                maxAttempts = 3,
                initialDelay = 1L,
                factor = 1.0
    ...
    fun withRetryThrowsOnUnknownError() = runTest {
        assertFailsWith<SparkError.Unknown> {
            withRetry(
                maxAttempts = 3,
                initialDelay = 1L,
                factor = 1.0
    ...
    fun withRetryExhaustsAllAttemptsOnPersistentOffline() = runTest {
        var attempts = 0
        assertFailsWith<SparkError.Offline> {
            withRetry(
                maxAttempts = 3,
                initialDelay = 1L,
                factor = 1.0
    ...
    fun withRetryExhaustsAllAttemptsOnPersistentServerError() = runTest {
        var attempts = 0
        assertFailsWith<SparkError.ServerError> {
            withRetry(
                maxAttempts = 3,
                initialDelay = 1L,
                factor = 1.0
...
class SafeCallTest {
    ...
    @Test
    fun safeCallMapsExceptionToSparkError() = runTest {
        val result = safeCall { throw OfflineException() }
        val error = result.exceptionOrNull()
        assertTrue(error is SparkError.Offline)
    }
    ...
    fun safeCallMapsNetworkExceptionToServerError() = runTest {
        ...
        val error = result.exceptionOrNull()
        assertTrue(error is SparkError.ServerError)
    }
    ...
    fun safeCallMapsRuntimeExceptionToUnknown() = runTest {
        ...
        val error = result.exceptionOrNull()
        assertTrue(error is SparkError.Unknown)
    }
    ...
    fun safeCallMapsSerializationExceptionToDataError() = runTest {
        ...
        val error = result.exceptionOrNull()
        assertTrue(error is SparkError.DataError)
    }
    ...
    fun safeCallUsesCustomErrorHandler() = runTest {
        ...
        assertTrue(result.isFailure)
        assertTrue(result.exceptionOrNull() is SparkError.Offline)
    }
...
// =============================================================================
// MARK: - Result (dev.spark.shared.util.Result) Tests
// =============================================================================
