...
class ErrorHandlerBranchCoverageTest {
    ...
    @Test fun mapSparkErrorPassesThrough() {
        val handler = DefaultErrorHandler()
    ...
    @Test fun mapNetworkException401() {
        ...
        assertTrue(error is SparkError.Unauthorized)
    }
    ...
    @Test fun mapNetworkException404() {
        ...
        assertTrue(error is SparkError.NotFound)
    }

    @Test fun mapNetworkException4xx() {
        val handler = DefaultErrorHandler()
        val error = handler.map(NetworkException(422, "Unprocessable"))
        assertTrue(error is SparkError.ClientError)
        assertEquals(422, (error as SparkError.ClientError).statusCode)
    }
    ...
    @Test fun toUserMessageOnSparkError() {
    ...
    @Test fun toUserMessageOnSparkErrorWithNullMessage() {
        val error = SparkError.Unknown(cause = null, message = "Something went wrong. Please try again.")
        val msg = error.toUserMessage()
        assertTrue(msg.isNotBlank())
    }
    ...
    @Test fun mapNetworkExceptionWithNullMessage() {
        ...
        assertTrue(error is SparkError.ClientError)
        assertEquals("Bad Request", (error as SparkError.ClientError).message)
    }

    @Test fun toUserMessageOnBilboDataError() {
        val msg = SparkError.DataError(cause = RuntimeException("test")).toUserMessage()
        assertTrue(msg.isNotBlank())
    }
    ...
    @Test fun toUserMessageOnBilboClientError() {
        val msg = SparkError.ClientError(400, "fail").toUserMessage()
        assertTrue(msg.isNotBlank())
    }
