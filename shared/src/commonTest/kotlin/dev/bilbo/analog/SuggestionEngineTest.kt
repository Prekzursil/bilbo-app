...
class SuggestionEngineGetSuggestionsTest {
    ...
    @Test fun getSuggestionsFiltersbyTime() =
        runTest {
            ...
            val engine =
                SuggestionEngine(
                    repo,
                    budgetRepo,
                    { setOf(SuggestionCategory.EXERCISE, SuggestionCategory.READING) },
                    { TimeOfDay.MORNING },
                )
            val result = engine.getSuggestions()
    ...
    @Test fun getSuggestionsFallsBackIfFewInterests() =
        runTest {
    ...
    @Test fun getSuggestionsSortsByTimesShownThenAcceptanceRate() =
        runTest {
            repo.suggestions.add(
                AnalogSuggestion(
                    id = 1,
    ...
    @Test fun getSuggestionsTakesTopN() =
        runTest {
            repeat(10) { i ->
                repo.suggestions.add(
                    AnalogSuggestion(
...
class SuggestionEngineAcceptTest {
    @Test fun acceptSuggestionRecordsAndAwardsFP() =
        runTest {
    ...
    @Test fun showAnotherRecordsShown() =
        runTest {
            val repo = FakeSuggestionRepo()
            val budgetRepo = FakeBudgetRepo()
            repo.suggestions.add(
...
class SuggestionEngineCustomTest {
    ...
    @Test fun addCustomSuggestionBlankTextFails() =
        runTest {
            val repo = FakeSuggestionRepo()
            val budgetRepo = FakeBudgetRepo()
    ...
    @Test fun deleteCustomSuggestion() =
        runTest {
            val repo = FakeSuggestionRepo()
            val budgetRepo = FakeBudgetRepo()
            repo.suggestions.add(
    ...
    @Test fun deleteNonExistentFails() =
        runTest {
            val repo = FakeSuggestionRepo()
            val budgetRepo = FakeBudgetRepo()
...
class SuggestionEngineAcceptanceRateTest {
    @Test fun acceptanceRateZeroShown() =
        runTest {
            val repo = FakeSuggestionRepo()
            val budgetRepo = FakeBudgetRepo()
