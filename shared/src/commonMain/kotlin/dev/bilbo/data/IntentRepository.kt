...
/** Read-only queries over [IntentDeclaration] records. */
interface IntentQueries {
    /**
...
/** Mutating operations over [IntentDeclaration] records. */
interface IntentMutations {
    /**
 ...
 *
 * Composed of [IntentQueries] (reads) and [IntentMutations] (writes) so neither
 * facet grows past the Strict-Zero interface-complexity ceiling.
 */
interface IntentRepository :
    ...
    suspend fun accuracyPercent(
        ...
        return (countAccurate(from, to).toFloat() / total.toFloat()) * PERCENT
    }
}
